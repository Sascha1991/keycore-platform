...
describe("AdminSupplierService", () => {
  it("exposes only the synthetic staging provider and creates a safe supplier", async () => {
    const fixture = serviceFixture();

    await expect(fixture.service.options(owner(), requestId)).resolves.toEqual([
      { label: "Synthetischer Testanbieter", type: "SYNTHETIC" },
    ]);
    await expect(
  ...
  it("fails closed outside staging and rejects invalid or conflicting input", async () => {
    const production = serviceFixture("PRODUCTION");
    await expect(
      production.service.options(owner(), requestId),
    ).resolves.toEqual([]);
    await expect(
      production.service.create(
        owner(),
        { displayName: "Supplier", operationId, providerType: "SYNTHETIC" },
        requestId,
      ),
    ...
    await expect(
      conflict.service.create(
        owner(),
        { displayName: "\u0000", operationId, providerType: "SYNTHETIC" },
        requestId,
      ),
  ...
  it("does not infer mutation authority from supplier read access", async () => {
    const fixture = serviceFixture();
    await expect(
      fixture.service.options(support(), requestId),
