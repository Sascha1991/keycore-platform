# Multi-Device-Entwicklung mit PC 1 und PC 2

Dieser Leitfaden ist die dauerhafte Arbeitsanweisung für die lokale
KeyCore-Entwicklung auf zwei unabhängigen Windows-Rechnern. `PC 1` und `PC 2`
...
| 7. PC wechseln         | PC 1 oder PC 2 | nach bestätigtem Push | physisch        | Von PC 1 zu PC 2 wechseln                                      | Übergabegrenze                          | GitHub enthält Commit                     | Ohne Push ist Arbeit nicht auf PC 2                                    |
| 8. PC-2-Zustand prüfen | PC 2           | vor Pull              | Repository-Root | `git status --short --branch`, `npm run dev:handoff`           | Lokale PC-2-Arbeit schützen             | Zustand verstanden                        | Bei Änderungen zuerst entscheiden/committen                            |
| 9. Aktualisieren       | PC 2           | bei sauberem Zustand  | Repository-Root | `git fetch --prune origin`, dann `git pull --ff-only`          | Commit übernehmen                       | Fast-forward erfolgreich                  | Bei Divergenz stoppen                                                  |
| 10. Start und Status   | PC 2           | nach Pull             | Repository-Root | `npm run dev:start`, dann `npm run dev:status`                 | Eigenen lokalen Stack nutzen            | PC-2-Dienste bereit                       | Abschnitte H bis K verwenden                                           |
...
| Schritt/Situation        | Rechner | Zeitpunkt                              | Wo?             | Befehl/Aktion                                        | Zweck                    | Erwartetes Ergebnis                          | Bei Fehler                              |
| ------------------------ | ------- | -------------------------------------- | --------------- | ---------------------------------------------------- | ------------------------ | -------------------------------------------- | --------------------------------------- |
| 1. Clone öffnen          | PC 2    | Sitzungsbeginn                         | PowerShell      | `cd <Pfad-zum-Clone>`                                | Richtige Arbeitskopie    | Root geöffnet                                | Pfad prüfen                             |
| 2. Zustand/Branch prüfen | PC 2    | vor Fetch                              | Repository-Root | `git status --short --branch`, `npm run dev:handoff` | Lokale Arbeit erkennen   | Branch korrekt, keine unerklärten Änderungen | Änderungen nicht blind stashen          |
| 3. Remote-Wissen holen   | PC 2    | nach Prüfung                           | Repository-Root | `git fetch --prune origin`                           | GitHub-Stand sehen       | Fetch erfolgreich                            | Netzwerk/Anmeldung prüfen               |
| 4. Sicher aktualisieren  | PC 2    | bei sauberem, nicht divergentem Branch | Repository-Root | `git pull --ff-only`                                 | Lineare Übergabe         | Aktuell/fast-forward                         | Abschnitt I                             |
| 5. Umgebung starten      | PC 2    | nach Pull                              | Repository-Root | `npm run dev:start`                                  | Eigene Volumes verwenden | Stack läuft                                  | Docker Desktop starten bzw. Logs prüfen |
| 6. Bereitschaft prüfen   | PC 2    | vor Arbeit                             | Repository-Root | `npm run dev:status`                                 | Dienste/URLs bestätigen  | Alles Erforderliche `OK`                     | Abschnitt H/J                           |

## E. Wechsel PC 2 zu PC 1

| Schritt/Situation    | Rechner | Zeitpunkt            | Wo?             | Befehl/Aktion                                                   | Zweck                      | Erwartetes Ergebnis                  | Bei Fehler                   |
| -------------------- | ------- | -------------------- | --------------- | --------------------------------------------------------------- | -------------------------- | ------------------------------------ | ---------------------------- |
| 1. Diff und Tests    | PC 2    | vor Wechsel          | Repository-Root | `git status --short`, `git diff --check`, passende Tests        | Qualität und Umfang prüfen | Nur beabsichtigte Arbeit             | Fehler beheben/dokumentieren |
| 2. Gezielt committen | PC 2    | nach Tests           | Repository-Root | `git add <dateien>`; Diff prüfen; `git commit -m "<Nachricht>"` | Sichere Übergabeeinheit    | Commit enthält keine lokalen Secrets | Staging-Diff korrigieren     |
| 3. Push und Prüfung  | PC 2    | vor Wechsel          | Repository-Root | `git push`, danach `npm run dev:handoff`                        | GitHub aktualisieren       | Ahead/Behind jeweils 0               | Kein Force-Push als Routine  |
| 4. Optional stoppen  | PC 2    | Arbeitsende          | Repository-Root | `npm run dev:stop`                                              | Ressourcen freigeben       | Daten bleiben erhalten               | Keine Volumes löschen        |
...
| 6. Fetch/Pull        | PC 1    | bei sauberem Zustand | Repository-Root | `git fetch --prune origin`; `git pull --ff-only`                | PC-2-Commit übernehmen     | Fast-forward                         | Divergenz bewusst klären     |
...
| Schritt/Situation                | Rechner        | Zeitpunkt   | Wo?             | Befehl/Aktion                                                    | Zweck                               | Erwartetes Ergebnis                | Bei Fehler                                                 |
| -------------------------------- | -------------- | ----------- | --------------- | ---------------------------------------------------------------- | ----------------------------------- | ---------------------------------- | ---------------------------------------------------------- |
| Fertige Arbeit                   | PC 1 oder PC 2 | Arbeitsende | Repository-Root | testen, gezielt stagen, committen und `git push`                 | GitHub-Backup/Handoff               | Remote synchron                    | Push-Fehler klären                                         |
| Unfertige, aber kohärente Arbeit | PC 1 oder PC 2 | Arbeitsende | Repository-Root | WIP-Commit auf dem Arbeitsbranch mit klarer Nachricht, dann Push | Sicherer geräteübergreifender Stand | Commit auf GitHub                  | Keine Secrets/kaputten gemeinsamen Branch ungeprüft pushen |
| Rein lokale, unfertige Arbeit    | PC 1 oder PC 2 | kurze Pause | Repository-Root | Dateien unverändert lassen und den PC nicht wechseln             | Kein künstlicher Commit             | Arbeit bleibt nur lokal            | Vor Gerätewechsel zwingend committen/pushen                |
| Laufzeit stoppen                 | PC 1 oder PC 2 | optional    | Repository-Root | `npm run dev:stop`                                               | Docker-Ressourcen freigeben         | Container gestoppt, Daten erhalten | Normalen Stop nicht durch Volume-Löschung ersetzen         |

## G. Tests und Prüfungen

| Schritt/Situation              | Rechner        | Zeitpunkt               | Wo?             | Befehl/Aktion                                               | Zweck                                                | Erwartetes Ergebnis   | Bei Fehler                      |
| ------------------------------ | -------------- | ----------------------- | --------------- | ----------------------------------------------------------- | ---------------------------------------------------- | --------------------- | ------------------------------- |
| Fokussierte Änderung           | PC 1 oder PC 2 | während Entwicklung     | Repository-Root | `npx vitest run <testdatei>`                                | Schnelles Feedback                                   | Betroffene Tests grün | Ursache im Scope beheben        |
| Normale Repository-Prüfung     | PC 1 oder PC 2 | vor Commit/Push         | Repository-Root | `npm run check`                                             | Format, Lint, Typen, Tests, Secret-Scan              | Alle Gates grün       | Nicht überspringen              |
| Plattformübergreifende Prüfung | PC 1 oder PC 2 | vor PR                  | Repository-Root | `npm run dev:check`                                         | `npm run check`, PHP/WordPress im Container, Compose | Alle drei Ebenen grün | Docker/konkretes Gate prüfen    |
| UAT-Struktur                   | PC 1 oder PC 2 | bei UAT-Doku            | Repository-Root | `npm run uat:validate`                                      | Matrixkonsistenz                                     | Validierung grün      | Keine Human-Ergebnisse erfinden |
| E2E/Security                   | PC 1 oder PC 2 | bei betroffenen Grenzen | Repository-Root | `npm run e2e:acceptance` bzw. `npm run security:assessment` | Breite Verhaltens-/Security-Evidenz                  | Suites grün           | Scope und Abhängigkeiten prüfen |

## H. Status, Logs und Diagnose

| Schritt/Situation | Rechner        | Zeitpunkt         | Wo?             | Befehl/Aktion                                                                                      | Zweck                            | Erwartetes Ergebnis          | Bei Fehler                                                                                       |
| ----------------- | -------------- | ----------------- | --------------- | -------------------------------------------------------------------------------------------------- | -------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------ |
| Gesamtstatus      | PC 1 oder PC 2 | jederzeit         | Repository-Root | `npm run dev:status`                                                                               | Git, Compose, Health, HTTP, URLs | Alle erwarteten Dienste `OK` | Betroffenen Dienst isolieren                                                                     |
| Begrenzte Logs    | PC 1 oder PC 2 | bei Fehler        | Repository-Root | `npm run dev:logs`                                                                                 | Letzte 200 Zeilen aller Dienste  | Diagnose ohne Endlosstream   | Docker Desktop prüfen                                                                            |
| Dienstlogs        | PC 1 oder PC 2 | bei Einzelproblem | Repository-Root | `npm run dev:logs -- admin`                                                                        | Admin gezielt prüfen             | Nur `keycore-admin`          | Erlaubte Aliase: `admin`, `storefront`, `wordpress`, `wordpress-db`, `postgres`, `redis`, `mail` |
| Rohstatus         | PC 1 oder PC 2 | tiefe Diagnose    | Repository-Root | `docker compose --env-file infra/docker/staging.local.env -f infra/docker/compose.staging.yaml ps` | Compose-Details                  | Containerzustände sichtbar   | Env/Pfad prüfen                                                                                  |

## I. Git-Fehler und Sonderfälle

| Schritt/Situation           | Rechner        | Zeitpunkt        | Wo?             | Befehl/Aktion                                                             | Zweck                      | Erwartetes Ergebnis                              | Bei Fehler                                                         |
| --------------------------- | -------------- | ---------------- | --------------- | ------------------------------------------------------------------------- | -------------------------- | ------------------------------------------------ | ------------------------------------------------------------------ |
| Uncommittete Änderungen     | PC 1 oder PC 2 | vor Pull/Wechsel | Repository-Root | `git status --short`; Diff einzeln prüfen                                 | Lokale Arbeit verstehen    | Klare Zuordnung                                  | Committen oder auf diesem PC weiterarbeiten; nicht blind verwerfen |
| Unerwartete untracked Datei | PC 1 oder PC 2 | vor Stage        | Repository-Root | Dateiname/Herkunft klären, `git check-ignore -v -- <pfad>`                | Secret-Aufnahme verhindern | Datei bewusst ignoriert oder bewusst aufgenommen | Kein pauschales `git add .`                                        |
| Falscher Branch             | PC 1 oder PC 2 | vor Arbeit       | Repository-Root | `git branch --show-current`; erst bei sauberem Tree `git switch <branch>` | Richtige Historie          | Erwarteter Branch                                | Änderungen vor Wechsel sichern                                     |
| Lokale Commits voraus       | PC 1 oder PC 2 | Übergabe         | Repository-Root | `npm run dev:handoff`; anschließend beabsichtigten Branch `git push`      | Commits veröffentlichen    | Ahead 0                                          | Upstream/Berechtigung prüfen                                       |
| Remote-Commits fehlen       | PC 1 oder PC 2 | Arbeitsbeginn    | Repository-Root | `git fetch --prune origin`; `git pull --ff-only`                          | Sicher aktualisieren       | Fast-forward                                     | Bei lokaler Arbeit stoppen                                         |
| Divergenz                   | PC 1 oder PC 2 | nach Fetch       | Repository-Root | `git log --oneline --left-right HEAD...@{upstream}`                       | Beide Seiten verstehen     | Ursache identifiziert                            | Nicht resetten/forcen; Rebase/Merge bewusst mit Reviewer planen    |
| Merge-/Rebase-Konflikt      | PC 1 oder PC 2 | Ausnahme         | Repository-Root | Konfliktdateien und Git-Status lesen                                      | Bewusste Auflösung         | Tests danach grün                                | Keine destruktive Einzeiler-Reparatur                              |
| GitHub nicht erreichbar     | PC 1 oder PC 2 | Fetch/Push       | Repository-Root | Netzwerk und `gh auth status`/Credential Manager prüfen                   | Ursache trennen            | Zugriff wiederhergestellt                        | Lokale Commits bleiben erhalten                                    |
| Push abgelehnt              | PC 1 oder PC 2 | Übergabe         | Repository-Root | `git fetch`; Divergenz prüfen                                             | Remote-Arbeit schützen     | Bewusste Synchronisierung                        | Kein `--force` als Routine                                         |

## J. Docker- und Startprobleme

| Schritt/Situation       | Rechner        | Zeitpunkt    | Wo?             | Befehl/Aktion                                                                                                  | Zweck                  | Erwartetes Ergebnis                   | Bei Fehler                                                    |
| ----------------------- | -------------- | ------------ | --------------- | -------------------------------------------------------------------------------------------------------------- | ---------------------- | ------------------------------------- | ------------------------------------------------------------- |
| Docker geschlossen      | PC 1 oder PC 2 | vor Start    | Windows         | Docker Desktop starten; `docker info`                                                                          | Engine bereitstellen   | Serverinformationen sichtbar          | WSL2/Virtualisierung/Installation prüfen                      |
| Port belegt             | PC 1 oder PC 2 | Startfehler  | PowerShell      | `Get-NetTCPConnection -State Listen` und gemeldeten Port zuordnen                                              | Kollision finden       | Besitzer bekannt                      | Fremden Dienst bewusst stoppen oder lokale Portwerte anpassen |
| Compose ungültig        | PC 1 oder PC 2 | Start/Check  | Repository-Root | `docker compose --env-file infra/docker/staging.local.env -f infra/docker/compose.staging.yaml config --quiet` | Env/Compose validieren | Exit 0                                | Fehlende Variable ergänzen, Datei nicht ersetzen              |
| Dienst unhealthy/exited | PC 1 oder PC 2 | Statusfehler | Repository-Root | `npm run dev:logs -- <alias>`                                                                                  | Konkrete Ursache       | Relevante bounded Logs                | Keine Volumes als erste Maßnahme löschen                      |
| Lokale Env fehlt        | PC 1 oder PC 2 | Startfehler  | Repository-Root | `npm run dev:setup`                                                                                            | Sicher erzeugen        | Datei entsteht ohne Ausgabe der Werte | Bestehende Datei nie überschreiben                            |

## K. Datenbank- und Migrationsprobleme

| Schritt/Situation        | Rechner        | Zeitpunkt             | Wo?             | Befehl/Aktion                                             | Zweck                                                                     | Erwartetes Ergebnis                | Bei Fehler                                           |
| ------------------------ | -------------- | --------------------- | --------------- | --------------------------------------------------------- | ------------------------------------------------------------------------- | ---------------------------------- | ---------------------------------------------------- |
| PostgreSQL nicht bereit  | PC 1 oder PC 2 | Start/Setup           | Repository-Root | `npm run dev:status`; `npm run dev:logs -- postgres`      | Health statt Port raten                                                   | Ursache sichtbar                   | Env/Disk/Docker prüfen                               |
| Migrationen prüfen       | PC 1 oder PC 2 | nach Pull             | Repository-Root | `npm run dev:setup`                                       | Bestehender Stack migriert über autoritativen Bootstrap und meldet Status | Migrationen current, keine Reseeds | Fehler stoppen; kein Auto-Rollback                   |
| Migration fehlgeschlagen | PC 1 oder PC 2 | Setup                 | Repository-Root | Ausgabe sichern, `npm run dev:logs -- postgres`           | Ursache analysieren                                                       | Keine weiteren Mutationen          | Nicht automatisch `db:rollback`/Volumes löschen      |
...
| Zustand unerwartet       | PC 1 oder PC 2 | jederzeit             | Repository-Root | Arbeit stoppen, Status/Dump-Metadaten dokumentieren       | Daten schützen                                                            | Ursache vor Mutation verstanden    | Keine Ad-hoc-SQL-Reparatur ohne Projektanweisung     |
...
Für die Nutzbarkeit bestimmter restaurierter Datensätze bestehen drei echte
kryptografische/Bootstrap-Abhängigkeiten. Falls die entsprechenden Daten auf
PC 2 weiterverwendet werden sollen, müssen ausschließlich
`KEYRANO_STAGING_GUEST_CLAIM_CODE`,
`KEYRANO_STAGING_BROWSER_MASTER_KEY` sowie
`KEYCORE_FULFILLMENT_MASTER_KEY` zusammen mit
`KEYCORE_FULFILLMENT_MASTER_KEY_ID` aus der lokalen PC-1-Konfiguration über
einen geschützten Kanal in die lokale PC-2-Konfiguration übertragen werden.
Der Guest-Claim-Wert wird zur Identitätsprüfung des festen synthetischen
Fixtures benötigt; die Master Keys werden zum Entschlüsseln bereits
persistierter synthetischer Fulfillment-Daten benötigt. Ohne passende Keys
bleibt verschlüsseltes Material absichtlich unlesbar. Keine Werte in Chat,
Screenshots, Shell-History, Git oder Dokumentation einfügen. Andere Secrets
werden nicht pauschal kopiert.

Ein PostgreSQL-Restore übernimmt den Hash der bestehenden
Admin-Passwortberechtigung. Das neu generierte PC-2-Passwort ersetzt ihn nicht,
weil normales Bootstrap keine Credentials rotiert. Statt das PC-1-Passwort zu
übertragen, verwendet der Human nach dem Restore den vorhandenen lokalen
`Passwort vergessen?`-Ablauf und Mailpit. WordPress-Anmeldungen bleiben dagegen
...
| Schritt/Situation               | Rechner        | Zeitpunkt                   | Wo?                                     | Befehl/Aktion                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Zweck                                                         | Erwartetes Ergebnis                                    | Bei Fehler                                                                              |
| ------------------------------- | -------------- | --------------------------- | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| 1. Git-Stand bestätigen         | PC 1 oder PC 2 | vor Dump                    | jeweiliger Root                         | Auf PC 1 und danach auf PC 2 jeweils `git rev-parse HEAD` ausführen                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Schema-/Code-Parität                                          | Identische SHA                                         | Erst normale Git-Übergabe abschließen                                                   |
...
| 7. Geschützt übertragen         | PC 1 oder PC 2 | nach Checksumme             | außerhalb Git                           | Dump bewusst von PC 1 zu PC 2 über einen privaten/verschlüsselten Datenträger übertragen                                                                                                                                                                                                                                                                                                                                                                                                                                           | Bewusster Datentransfer                                       | Datei auf PC 2                                         | Nie committen oder öffentlich teilen                                                    |
...
| 9. Erforderliche Konfiguration  | PC 2           | vor Restore                 | lokaler Secret-Editor/geschützter Kanal | Nur die oben genannten vier Variablen sicher von PC 1 nach `infra/docker/staging.local.env` auf PC 2 übertragen; Werte nie in Befehle/Chat einfügen                                                                                                                                                                                                                                                                                                                                                                                | Restaurierte Fixture-/Ciphertext-Kompatibilität               | Namen vorhanden, Werte bleiben verborgen               | Bei fehlendem passenden Wert Restore verschieben oder Daten als nicht nutzbar behandeln |
...
| Schritt/Situation            | Rechner        | Zeitpunkt | Wo?             | Befehl/Aktion                                                                                                                     | Zweck                                                | Erwartetes Ergebnis                            | Bei Fehler                                  |
| ---------------------------- | -------------- | --------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------- | ------------------------------------------- |
| `package-lock.json` geändert | PC 1 oder PC 2 | nach Pull | Repository-Root | `npm ci` oder umfassend `npm run dev:setup`                                                                                       | Exakte Abhängigkeiten                                | Lockfile unverändert, Installation erfolgreich | npm-/Node-Version prüfen                    |
| Migration hinzugekommen      | PC 1 oder PC 2 | nach Pull | Repository-Root | `npm run dev:setup`                                                                                                               | Pending Migration über vorhandene Autorität anwenden | DB current                                     | Kein Reset/Auto-Rollback                    |
| Dockerfile/Compose geändert  | PC 1 oder PC 2 | nach Pull | Repository-Root | `npm run dev:start`                                                                                                               | Images neu bauen                                     | Dienste bereit                                 | `dev:logs`, Compose config                  |
| Env-Template geändert        | PC 1 oder PC 2 | nach Pull | Repository-Root | `npm run dev:start` bzw. `dev:setup` zur Validierung; fehlende Variable gezielt aus `infra/docker/staging.env.example` übernehmen | Bestehende Secrets bewahren                          | Validierung ohne Template-Drift                | Env nie komplett überschreiben              |
| WordPress-Bootstrap geändert | PC 1 oder PC 2 | nach Pull | Repository-Root | Änderung/Release-Hinweis prüfen; nur bei ausdrücklich sicherer/idempotenter Anweisung den dokumentierten Bootstrap ausführen      | Human-Inhalte schützen                               | Bewusste Aktualisierung                        | Nicht automatisch auf Review-Daten anwenden |

## N. Checklisten PC 1 und PC 2

### Bevor ich anfange

- Im richtigen Repository: `git status --short --branch`.
- Richtiger Arbeitsbranch und erwarteter Upstream: `npm run dev:handoff`.
- Nach sicherem Fetch mit `git pull --ff-only` synchronisiert.
- Docker Desktop läuft; `npm run dev:start` und `npm run dev:status` sind grün.

### Bevor ich den Rechner wechsle

- Beabsichtigte Dateien und Diff geprüft; keine Secrets/unbekannten Dateien.
- Angemessene Tests erfolgreich.
- Gezielt gestaged, Commit erstellt und Push erfolgreich.
- `npm run dev:handoff` meldet Ahead/Behind jeweils 0.
- Optional `npm run dev:stop`; Daten bleiben auf diesem PC.
...
## Mit Codex auf PC 1 oder PC 2 weiterarbeiten

Ein frischer Codex-Task benötigt keinen Export dieses Chats. Empfohlener erster
Auftrag:

> Lies `AGENTS.md` und
> `docs/development/MULTI-DEVICE-DEVELOPMENT.md`. Prüfe danach Branch, HEAD,
> Upstream und Working Tree. Ändere nichts, bevor der aktuelle Projektstand und
> der konkrete Auftrag geklärt sind.
